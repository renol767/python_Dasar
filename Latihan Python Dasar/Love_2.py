...
def create_window(name):
    ...
    w = AnimatedSketchWindow(name=f"window_glitch_{name}",
                             x=randint(-50, winsize[1]),
                             y=window_y,
                             w=randint(100, 2000),
                             h=window_h,
                             bg=0)
    w.vx = randint(100, 400)
    w.vy = randint(-50, 50)
    w.bg = 0 if randint(0, 100) < 50 else 255 
    return w


glitch_windows = [create_window(i) for i in range(25)]

while True:
    for window in glitch_windows:
        window.bg = 0 if randint(0, 100) < 50 else 255
        window.mask_img(ImageMask(filename="./Latihan Python Dasar/bila.jpg"))
        window.draw()
        window.update()
